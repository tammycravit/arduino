...
/* 
Modified slightly by Andrew Mowry 2015 to make it work with MintySynth (mintysynth.com)

The only changes I've made are assigning the correct pins for MintySynth and changing/minimizing the LED usage to make the most of MintySynth's two LEDs. Otherwise the functionality is the same.

The red and yellow leds flash on notes 0 and 8 respectively when in Edit Mode. They also briefly indicate whether the Red page or Yellow page is selected in preferences.

The LEDs also count in binary (repeating after 4) when using a pot to select a discrete value (in Pattern Mode when selecting bank and follow mode, and in Preferences when selecting values with the pots etc.):

Selected value   Red Yellow
       1           o   o
       2           o   x
       3           x   o
       4           x   x
...
//const byte LEDlatchPin = 12;
//const byte LEDclockPin = 12;
//const byte LEDdataPin = 12;
...
byte buttons[] = {
  5, 7, 11, 8, 10}; // the analog 0-5 pins are also known as 14-19
...
void loop() {
 ...
 // ledsOff();
  ...
  //  {
   ...
   // && potLock[4] < 9999 && bank >= 8
   
   
   if (!pressed[4] && mode ==1) 
 {showBinary(bank);} //Show the bank number in binary, repeating after 4 (added by A.M. for MintySynth).
    ...
    else // shift is held
    {
      ...
      if (potLock[4] == 9999 && bank < 8)
      {
        ...
        if (mode == 1) {showBinary(followMode);}  //Show the followMode in binary (added by A.M. for MintySynth).
      }
      else if (difference(potLock[4], pot[4]) > 8)
        potLock[4] = 9999; 
          ...
          if (potLock[i] == 9999)
          {
            ...
            case 0:
             ...
             if (valueDisplayTimer != 0)
              ...
              {showBinary(noteChannel);} //Added by A.M.
              break;
            case 1:
             ...
             showBinary(triggerChannel); //Added by A.M.
...
if (savealert)
 ...
 if (seqCurrentStep == 0 && mode == 0){
 digitalWrite(2,HIGH);}
 else if (seqCurrentStep != 0 && mode == 0) {digitalWrite(2,LOW);}
 
  if (seqCurrentStep == 8 && mode == 0){
 digitalWrite(4,HIGH);}
 else if (seqCurrentStep != 8 && mode == 0) {digitalWrite(4,LOW);}
}
